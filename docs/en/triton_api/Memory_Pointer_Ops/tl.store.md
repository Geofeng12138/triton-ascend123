...
| `eviction_policy` | `str`               | Controls the eviction policy on NVIDIA PTX, has no effect on Ascend hardware |
...
3. If `pointer` is from `tl.make_block_ptr`:
   - `mask` must be `None`
   - `boundary_check` can be used to set boundary checks
...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- |
| Generalization issue with discrete masks  | Currently, the handling of discrete masks in `store` involves decomposing the store into atomic {load, select, store}, which has generalization issues in corner cases | Expose issues through extensive generalization testing, resolve iteratively |
| Generalization issue with loops and branches | Currently, the computation of `pointer` and `mask` in `tl.load` may cause compilation issues if it involves complex loops and branch statements | Expose issues through extensive generalization testing, resolve iteratively |

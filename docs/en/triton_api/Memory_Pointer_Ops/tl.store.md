...
| ---------------- | ------------------- | ----------------------------------------------------------------------- |
| `pointer`        | `triton.PointerType` <br> or `tensor<triton.PointerType>` <br> or `triton.PointerType<tensor>` (from `tl.make_block_ptr`) | Pointer to the address in GM where the value will be stored |
| `value`          | `tensor` or `scalar` | The value to be stored, supports implicit broadcasting and implicit type conversion |
| `mask`           | `int1` or `tensor<int1>` | Optional parameter, can only be passed when `pointer` is not from `tl.make_block_ptr`.<br>If `mask[i]==False`, `value[i]` will not be stored to the address pointed to by `pointer[i]`; if `True`, it is stored normally.<br>If `pointer` is from `tl.make_block_ptr`, `mask` must be `None`. |
| `boundary_check` | `tuple(int)` | Optional parameter, can only be passed when `pointer` is from `tl.make_block_ptr`.<br>An integer tuple indicating the dimensions that require boundary checking. |
| `cache_modifier` | `""` or `"ca"` or `"cg"` | Optional parameter, controls the cache option on NVIDIA PTX, has no effect on Ascend hardware. |
...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| Generalization issue with discrete mask  | The current handling of discrete masks in `store` decomposes the store into atomic {load, select, store}, which has generalization issues in corner cases. | Expose issues through extensive generalization testing, iterative resolution. |
| Generalization issue with loops and branches | The current computation of `pointer` and `mask` in `tl.load` may encounter compilation issues if it involves complex loops and branch statements. | Expose issues through extensive generalization testing, iterative resolution. |

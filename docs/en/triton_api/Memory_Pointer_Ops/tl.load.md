...
Description: Returns a Tensor/Scalar whose values are loaded from the location pointed to by the `pointer` argument in GlobalMemory.
...
| ------------- | ----------------- | -------------------------------------------------------------- |
| `pointer` | `triton.PointerType` <br> or `tensor<triton.PointerType>` <br> or `triton.PointerType<tensor>` (from `tl.make_block_ptr`) | Pointer to the data to be read in GM |
...
| `padding_option` | `""` or `"zero"` or `"nan"` | Optional parameter, can only be passed when `boundary_check` is not empty<br>Indicates the padding value when access is out of bounds |
| `cache_modifier` | `""` or `"ca"` or `"cg"` | Optional parameter, controls the cache option on NVIDIA PTX, ineffective for Ascend hardware |
| `eviction_policy` | `str` | Controls the eviction policy of NVIDIA PTX, ineffective for Ascend hardware |
| `volatile` | `str` | Controls the volatile option of NVIDIA PTX, ineffective for Ascend hardware |
...
| Ascend A2/A3 | Supports scalar and 1~5 dimensional tensors |
...
3. If `pointer` is from `tl.make_block_ptr`:
   ...
   - `boundary_check` and `padding_option` can be used to set boundary checks and out-of-bounds padding values
...
| Generalization issue when used with branch and loop statements | Currently, the computation process of `pointer` and `mask` in `tl.load`, if it involves complex loops and branch statements, may cause compilation issues | Expose issues through extensive generalization testing, resolve iteratively |

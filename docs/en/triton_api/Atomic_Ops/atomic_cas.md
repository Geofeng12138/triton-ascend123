...
Description: Atomic compare-and-swap operation. It compares the value at *pointer with `cmp`. If they are equal, *pointer is updated to `val`; otherwise, *pointer remains unchanged.
Prototype:
...
| -------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `pointer`      | `triton.PointerDType` | The memory location to operate on. If *pointer == cmp, *pointer is updated to `val`. The computed result is written back to this memory. |
...
| `sem`                   | Community official acceptable values are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed".<br>We only support "acq_rel". |

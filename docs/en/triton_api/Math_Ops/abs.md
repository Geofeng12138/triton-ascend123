...
It can be called as a member function of a tensor, e.g., `x.abs()`, which is equivalent to `abs(x)`.
...
The following example performs the absolute value operation on the input tensor `x`:

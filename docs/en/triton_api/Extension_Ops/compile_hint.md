...
2. **`list` parameter only supports integer arrays**: Elements must be integers (`int` or `constexpr` integers); floating-point numbers or mixed-type lists are not supported.

...
1. **`hint_name` must be a string type**: Other types cannot be passed as the hint name
2. **The `list` parameter only supports integer arrays**: Elements must be integers (`int` or `constexpr` integers); floating-point numbers or mixed-type lists are not supported
3. **Non-intrusive design**: `compile_hint` does not change computation semantics; it only adds metadata
4. **A single tensor can be annotated multiple times**: The same tensor can have multiple hints with different names attached

...
| `return_indices` | `bool` | If true, return the index corresponding to the maximum value, in addition to the maximum value itself |
| `return_indices_tie_break_left` | `bool` | If multiple elements have the same maximum value, return the left-most index for values that aren’t NaN |

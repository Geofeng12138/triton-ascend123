...
Description: Returns the indices of the maximum values along a specified dimension.
...
| `axis`   | `int` | Specifies the dimension along which to perform the reduction |
| `keep_dims` | `bool` | Whether to keep the reduced dimension |
| `tie_break_left` | `bool` | If multiple elements have the same maximum value, returns the index of the leftmost maximum value |
...
For the case of tensor[nan,inf], returns the index of inf.
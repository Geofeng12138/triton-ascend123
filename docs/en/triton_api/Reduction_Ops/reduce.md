...
| `combine_fn` | `Callable` | A function used to combine two groups of scalar tensors (must be decorated with `@triton.jit`) |
...
> `keep_dims=True` requires testing more specifications to determine full support. Currently, it is confirmed that `keep_dims=True` is supported for 3D with dim=2.

### 2.4 Usage

The following example implements a reduce computation on a 2D shape tensor, where the `combine_fn` uses simple addition:

...
| `axis` | `int` or `None` | The dimension along which to compute the XOR sum. If `None`, the XOR operation is performed over all dimensions |
...
> `keep_dims=True` requires more specification testing to determine full support. Currently, it has been tested for 3D with `dim=2` and supports `keep_dims=True`.

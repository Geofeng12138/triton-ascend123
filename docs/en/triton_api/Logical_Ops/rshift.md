...
1. Ascend lacks support for unsigned integer types compared to GPU.
2. The right operand `other` only supports scalars, not tensors (i.e., `x >> 2` is valid, but `x >> y` (where `y` is a tensor) is not supported).

### 2.4 Usage Example

The following example implements a right-shift operation on three-dimensional tensors `x0` and `x1`:

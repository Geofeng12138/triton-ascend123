...
Description: **Converts the indices of a row-major matrix of size size_i × size_j, grouped by every size_g rows, into the indices of a column-major matrix.**
...
The following example converts the indices of a row-major matrix grouped by every `size_g` rows into the indices of a column-major matrix:

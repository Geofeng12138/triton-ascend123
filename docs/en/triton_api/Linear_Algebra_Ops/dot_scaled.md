...
| `lhs_k_pack`   | `(bool, optional)` | true packs along the K dimension<br>false packs along the M dimension |
| `rhs_k_pack`   | `(bool, optional)` | true packs along the K dimension<br>false packs along the N dimension |
...
2. The scale tensor value is int8 on Ascend and uint8 on GPU.
...
1. Due to lack of fp8 support, the left and right matrices do not support fp4 and fp8 formats. Ascend lacks the matrix decompression support capability for `lhs_k_pack` and `rhs_k_pack` compared to GPU (hardware limitation).

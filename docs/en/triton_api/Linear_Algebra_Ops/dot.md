...
Description: Performs matrix multiplication on two tensors. Tensors must be two-dimensional or three-dimensional with consistent dimensions. For three-dimensional blocks, `tl.dot` performs batched matrix multiplication, where the first dimension of each block represents the batch dimension.
Prototype:
...
Return Value:
`tl.tensor`: Result of the matrix multiplication.
...
- `out_dtype` lacks support for `int8` and `fp16` types compared to GPU.

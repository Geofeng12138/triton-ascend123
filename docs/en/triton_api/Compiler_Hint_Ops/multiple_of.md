...
**`values` describes the divisibility characteristics of the first value in each dimension, so the dimension of `values` must be the same as the dimension of `input`.

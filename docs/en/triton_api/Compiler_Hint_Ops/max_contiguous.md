...
| `values` | `constexpr[int]` or `list[constexpr[int]]` | Required | Compile-time constant integer (or list of integers) describing the contiguity pattern |
| `_semantic` | - | - | Reserved parameter, not supported for external calls |

**`values` describes the contiguity characteristics of each dimension, so the dimensionality of `values` must match the dimensionality of `input`.

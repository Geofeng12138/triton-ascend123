...
Return value:
`tensor`: A tensor of the contiguous integer sequence

### 2.2 Supported Specifications

#### 2.2.1 DataType Support

Conclusion: The parameters `start` and `end` of arange must be constants, therefore they have no type. The supported type corresponds to the value range, up to int32. Hardware instructions also only support up to int32.
...
3. The input of arange must be a constant, supporting uint and int type values less than 1048576 (maximum TRITON_MAX_TENSOR_NUMEL). int64 is not supported.

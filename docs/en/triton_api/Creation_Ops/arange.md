...
Conclusion: The parameters `start` and `end` of arange must be constants, therefore they have no type. Supported types correspond to their value ranges, up to int32. Hardware instructions also only support up to int32.
...
> Relative community feature missing and unimplementable

1. The function generates a contiguous integer sequence [start, end). CUDA requires that range = (end - start) must be a power of 2. Triton-ascend has no such requirement.
2. Both NV and Triton-ascend limit the maximum value of end to TRITON_MAX_TENSOR_NUMEL = 1048576.
3. The inputs to arange must be constants, supporting uint and int type values less than 1048576 (the maximum value TRITON_MAX_TENSOR_NUMEL). int64 is not supported.

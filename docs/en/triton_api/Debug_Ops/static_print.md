...
If printing **non-constant** results, it will print a value in the format `data_type[data_shape(scalar is empty)]`. For example, if the data type pointed to by `x_ptr` is `int32`, it will print `val:int32[constexpr[4]]`:

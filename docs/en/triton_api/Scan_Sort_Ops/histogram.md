...
Description: Computes a histogram with `num_bins` bins based on `input`, where each bin has a width of 1 and starts at 0.
Prototype:
...
| `_generator`   | -                  | Reserved parameter, external calls not supported temporarily     |
...
> Relative community capability missing but can be developed and supported
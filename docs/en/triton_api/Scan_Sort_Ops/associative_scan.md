...
| `reverse` | `bool` | Whether to apply the associative scan along the reverse direction of the axis |
...
> `reverse=True` indicates whether to apply the associative scan along the reverse direction of the axis. This feature requires alignment when loading data with `tl.load`, i.e., not using a mask to filter out excess data indices, as shown in the following example code:
...
The following example implements the `associative_scan` operation on a 2D-shaped tensor:

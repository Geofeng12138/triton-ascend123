...
The `cumsum` function computes the cumulative sum (prefix sum) along the specified axis. For example, for the input `[a, b, c, d]`, the cumulative sum result is `[a, a+b, a+b+c, a+b+c+d]`.
